...
public class Loic extends JavaPlugin {
    ...
    private final LoicPlayerListener playerListener 
            = new LoicPlayerListener(this);
    private final LoicBlockListener blockListener 
            = new LoicBlockListener(this);
    private final LoicEventListener eventListener 
            = new LoicEventListener(this);
    ...
    private void registerEvents() {
        getServer().getPluginManager().registerEvent
                (Event.Type.PLAYER_ANIMATION, this.playerListener, 
                Priority.Normal, this);
        getServer().getPluginManager().registerEvent
                (Event.Type.ENTITY_EXPLODE, this.blockListener, 
                Priority.Normal, this);
        getServer().getPluginManager().registerEvent
                (Event.Type.ENTITY_EXPLODE, this.eventListener, 
                Priority.Normal, this);
    }
        ...
        if (command.getName().toString().equalsIgnoreCase("ioncannon")) {
            ...
            if (cannons.containsKey(((Player)sender).getName().toString())) {
                getServer().broadcastMessage(ChatColor.YELLOW + 
                        "ATTENTION : " + ChatColor.BLUE + 
                        "Ion Cannons has been disabled");
            ...
            if (size < 1)
                size = 1;
            getServer().broadcastMessage(ChatColor.RED + "WARNING : " 
                    + ChatColor.BLUE + "Ion Cannon Primed.");
            getServer().broadcastMessage(ChatColor.YELLOW + "ATTENTION : " 
                    + ChatColor.BLUE + "Beam Size Locked at " +ChatColor.GOLD 
                    + size + ".");
            
            cannons.put(((Player)sender).getName().toString(), 
                    Integer.valueOf(size));
            return true;
        }
        
      log.info("[WARNING] Player " + sender.toString() + 
              " was denied access to the command: " + command.toString() + ".");
    ...
    return false;
     ...
     private void placePoints(int cx, int cz, int x, int z, int y, 
             int blockType, Player player) {
     if (x == 0) {
  ...
  private void placeCylinderPoints(int cx, int cz, int x, int z, int y, 
          int blockType, Player player) {
    if (x == 0) {
  ...
  public void circleMidpoint(int xCenter, int yCenter, int radius, int y, 
          int blockType, Player player) {
    int x = 0;
    int z = radius;
    int p = (5 - radius * 4) / 4;
  ...
  public void cylinderModpoint(int xCenter, int yCenter, int radius, int y, 
          int blockType, Player player) {
    int x = 0;
    int z = radius;
    int p = (5 - radius * 4) / 4;
  ...
  public void armSwing(Player player) {
    if (cannons.containsKey(player.getName().toString())) {
      ...
      if (block == null)
        return;
      getServer().broadcastMessage(ChatColor.YELLOW + "ATTENTION : " 
              + ChatColor.BLUE + "Ion Cannon Target Locked.");
      getServer().broadcastMessage(ChatColor.RED + "WARNING    : " 
              + ChatColor.BLUE + "Ion Cannon Firing...");
      int size = ((Integer)cannons.get(player.getName())).intValue();
      log.info("[" + name + "] IonCannon fired by: '" 
              + player.getName().toString() + "' with size: " + size);
      ...
      player.getWorld().getBlockAt(block.getX(), 127, 
              block.getZ()).setTypeId(51);
    }
  }
}